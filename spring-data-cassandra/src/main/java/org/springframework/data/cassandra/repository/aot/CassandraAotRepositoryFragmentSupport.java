 ...
 * @since 5.0
 */
public class CassandraAotRepositoryFragmentSupport {
	...
	protected CassandraAotRepositoryFragmentSupport(CassandraOperations cassandraOperations,
			RepositoryFactoryBeanSupport.FragmentCreationContext context) {
		this(cassandraOperations, context.getRepositoryMetadata(), context.getProjectionFactory());
	}

	protected CassandraAotRepositoryFragmentSupport(CassandraOperations cassandraOperations, RepositoryMetadata repositoryMetadata,
			ProjectionFactory projectionFactory) {

		this.cassandraOperations = cassandraOperations;
		this.cassandraConverter = cassandraOperations.getConverter();
		this.repositoryMetadata = repositoryMetadata;
		this.projectionFactory = projectionFactory;
	}

	protected Document bindParameters(String source, Object[] parameters) {
		return new BindableCassandraExpression(source, this.cassandraConverter, parameters).toDocument();
	}
	...
	protected List<Object> convertSimpleRawResults(Class<?> targetType, List<Document> rawResults) {
		...
		for (Document it : rawResults) {
			list.add(extractSimpleTypeResult(it, targetType, cassandraConverter));
		}
		return list;
	}
	...
	private static <T> T getPotentiallyConvertedSimpleTypeValue(CassandraConverter converter, @Nullable Object value,
			Class<T> targetType) {
