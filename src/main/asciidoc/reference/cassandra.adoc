...
such as `com.datastax.driver.core.Session`, which lets you communicate directly with Cassandra.
...
Spring Data for Apache Cassandra requires Apache Cassandra 2.1 or later, Datastax Java Driver 3.0 or later,
and Java SE 8 or later. An easy way to quickly set up and bootstrap a working environment is to create
...
.Registering a com.datastax.driver.core.Session object by using Java-based bean metadata
====
[source,java]
----
@Configuration
...
An alternative is to register an instance of `com.datastax.driver.core.Session` with the container by
...
.Registering a com.datastax.driver.core.Session object by using Spring's CassandraCqlSessionFactoryBean and enabling Spring's exception translation support
====
[source,java]
----
@Configuration
...
`com.datastax.driver.core.Session` with the container, the XML can be quite verbose, because it is general purpose.
XML namespaces are a better alternative to configuring commonly used objects, such as the `Session` instance.
The `cql` and `cassandra` namespaces let you create a `Session` instance.

The following example shows how to configure the `cql` namespace:

.XML schema to configure Cassandra by using the `cql` namespace
====
[source,xml]
----
...
NOTE: You may have noticed the difference between namespaces: `cql` and `cassandra`. Using the `cql` namespace
...
Startup and shutdown CQL execution follows a slightly different approach that is bound to the `Cluster` lifecycle.
You can provide arbitrary CQL that is executed on `Cluster` initialization and shutdown in the `SYSTEM` keyspace, as the following XML example shows:

.Specifying Startup and Shutdown scripts with XML
====
[source,xml]
----
<cql:cluster>
...
.Specifying Startup and Shutdown scripts with Java configuration
====
[source,java]
----
@Configuration
...
NOTE: Both the `cql` and `cassandra` namespaces include `KeyspaceSpecifications` and the lifecycle CQL scripts.
...
of all of the functionality exposed by the `CqlTemplate`. See the https://docs.spring.io/spring-data/cassandra/docs/current/api/[Javadoc] for that.
...
==== Updating `INSERT`, `UPDATE`, and `DELETE` with `CqlTemplate`
...
Applications connect to Apache Cassandra by using `Cluster` and `Session` objects. A Cassandra `Session` keeps track of
multiple connections to the individual nodes and is designed to be a thread-safe, long-lived object.
Usually, you can use a single `Session` for the whole application.
...
NOTE: `CassandraTemplate` has different execution model flavors. The basic `CassandraTemplate` uses a
...
You can also lookup the `CassandraTemplate` bean from the `ApplicationContext`, as shown in the following example:
...
The simple case of using the insert operation is to save a POJO. In this case, the table name is determined
by the simple (not fully-qualified) name of the class. The table in which to store the object can be overridden
by using mapping metadata.

When inserting or updating, the `id` property must be set. Apache Cassandra has no means with which to generate an ID.
...
You can manage the collection name that is used for operating on the tables in two ways. The default table name
