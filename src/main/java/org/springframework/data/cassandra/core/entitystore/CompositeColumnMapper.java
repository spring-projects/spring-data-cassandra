...
class CompositeColumnMapper extends AbstractColumnMapper {
	...
	CompositeColumnMapper(final Field field) {
		...
		ColumnFamily entityAnnotation = clazz.getAnnotation(ColumnFamily.class);
		if(entityAnnotation == null)
			throw new IllegalArgumentException("class is NOT annotated with @org.springframework.data.casandra.entitystore.ColumnFamily: " + clazz.getName());
		...
		for (Field childField : clazz.getDeclaredFields()) {
			...
			if ((annotation != null)) {
				...
				ColumnFamily compositeAnnotation = childField.getType().getAnnotation(ColumnFamily.class);
