...
public class UserTypeSpecification<T> extends UserTypeNameSpecification<UserTypeSpecification<T>> implements UserTypeDescriptor {
	...
	/**
	 * Adds the given frozen column to the type.
	 * 
	 * @param name The column name; must be a valid unquoted or quoted identifier without the surrounding double quotes.
	 * @param type The data type of the column.
	 */
	public T frozenColumn(String name, DataType type) {
		return column(cqlId(name), type, true);
	}
	 ...
	 * @param type The data type of the column.
	 */
	public T frozenColumn(CqlIdentifier name, DataType type) {
		return column(name, type, true);
	}

	@SuppressWarnings("unchecked")
	protected T column(CqlIdentifier name, DataType type, boolean frozen) {
		ColumnSpecification column = new ColumnSpecification().name(name).type(type).frozen(frozen);
		columns.add(column);
		return (T) this;
	}
